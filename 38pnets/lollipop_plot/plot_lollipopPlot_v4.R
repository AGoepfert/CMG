...
create_Lollipop_Plot <- function(geneStructure, data,single.gene,fileName){
  ...
  legend <- 1:4 ## legend fill color
  names(legend) <- paste0("legend", letters[1:4]) 
  
  legend  <- list(labels=paste0("legend", LETTERS[1:4]), 
                  col=palette()[4:1], 
                  fill=palette()[legend])
  ...
  #x11()
  plot.new()
  lolliplot(mutation.SNP, c(features,features2,features3))
  ...
  library(gridExtra)
  plot.new()
  grid.table(data[,which(!apply(data,2,FUN = function(x){all(x == 0)}))]) # removes all columns/ samples with no SNP
...
for (y in files){
  ...
  for (single.gene in input_genes){
    ...
    create_Lollipop_Plot(geneStructure,input,single.gene,y)
    i=i+1
    break
    if(i>2){
      break
