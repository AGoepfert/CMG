...
getGene <- function(geneName){
  ...
  if(nrow(listCompleteTranscripts)>0){
    ...
    #listCompleteTranscripts=FALSE
    grid.text(paste(single.gene," - Chr.",geneLocations$chromosome_name[1], "- NO gene informations available",sep=""), x=.5, y=.01, just="bottom")
  }
...
create_Lollipop_Plot <- function(geneStructure, data,single.gene,fileName,normalSamples){
  ...
  normal_tumor.gr$color <- rep(list(c('purple', 'green')), length(SNP))
  normal_tumor.gr$value1 <- as.double(as.vector(data$countNormal))
  normal_tumor.gr$value2 <- as.double(as.vector(data$countTumor))
  
  legends <- list(list(labels=c("Normal", "Tumor"), fill=c('green', 'purple')), 
                  list(labels=unique(data$mutationtype), fill=unique(data$colorMutationType)))
  
  
  lolliplot(list(A=normal_tumor.gr, B=mutation.SNP), 
            list(x=features.mul, y=features.mul), 
            type=c("pie","circle"), legend=legends,ylab=c("normal & tumor amount","SNP"),
            cex=.75)
  
  
  grid.text(paste(single.gene," - Chr.",geneLocation.Chr,sep=""), x=.5, y=.01, just="bottom")
  grid.text(paste(fileName), x=.5, y=.98, just="top",
...
for (y in files){
  ...
  for (single.gene in input_genes){
    ...
    if (nrow(as.data.frame(geneStructure[1]))>0){
