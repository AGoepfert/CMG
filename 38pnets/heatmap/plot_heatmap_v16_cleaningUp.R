...
createHeatmap_tumorNormal <- function(samples,pairedSamples_list,data_matrix,plotTitle){
  tm_SamplesMatrix <- samples
  colnames(data_matrix) <- gsub("_VSC ","", colnames(data_matrix))
  ...
  tm_SamplesMatrix <- tm_SamplesMatrix[which(tm_SamplesMatrix[,"tm_samples"] %in% pairedSamples_list),]
  rownames(tm_SamplesMatrix) <- tm_SamplesMatrix[,"tm_samples"]
  
  tm_SamplesMatrix <- tm_SamplesMatrix[pairedSamples_list,]
  
  data_matrix<- data_matrix[,pairedSamples_list]
  ...
  used.table <- data_matrix[,c(1:(ncol(data_matrix)-6))]
  used.table <- used.table[, order(colnames(used.table))] 

  pdf(paste("pairedSamples_",plotTitle,sep=""))
  
  ##Add Annotation about samples
  df = data.frame(stage = tm_SamplesMatrix[,'tm_stage'], grade = tm_SamplesMatrix[,'tm_grade'])
  
  ###### STAGE
  colfunc <- colorRampPalette(c( "mediumseagreen","orangered3","orange"))
  listOfColors <- colfunc(length(unique(tm_SamplesMatrix[,'tm_stage'])))
  stagelist  <- listOfColors
  names(stagelist) <- unique(tm_SamplesMatrix[,'tm_stage'])
  
  ###### GRADE
  colfunc <- colorRampPalette(c( "mediumseagreen","royalblue4","lightsteelblue1"))
  listOfColors <- colfunc(length(unique(tm_SamplesMatrix[,'tm_grade'])))
  gradelist  <- listOfColors
  names(gradelist) <- unique(tm_SamplesMatrix[,'tm_grade'])
  ...
  heatmap= Heatmap(scale(used.table, center=FALSE, scale=FALSE),na_col ="lightsteelblue1", name="Allelic fraction",
                   col = colorRamp2(c(0,0.05,0.1,0.4,1), c("aliceblue","deepskyblue4" ,"mediumaquamarine","orange","red")),
                   row_title ="", 
                   row_title_side ="right",
                   row_title_rot = 0,
...
#3 Input.matrix with all normal files
pairedSamples_list <- c("ch12","ch11","r3n","r3t","r4n","r4t")

#4 Input.matrix with all normal files
samples <- rbind(tm_SamplesMatrix,normalSamplesMatrix)
...
for (y in files){
  ...
  createHeatmap_tumorNormal(samples,pairedSamples_list,data_matrix,plotTitle)
  #break
}
